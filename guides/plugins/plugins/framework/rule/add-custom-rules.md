...
We'll explain what `notEquals` and `equalsAny` means later. Now we just need a new snippet.

That's it, you successfully defined restrictions for your custom condition. If you now try to assign a rule with your condition to advanced prices, you should see that it's not possible, and the rule is disabled.
...
For that, we need to write some twig code. The important property here is the `rule-aware-group-key` property which should match the assigment name of the rule aware group we just extended.
